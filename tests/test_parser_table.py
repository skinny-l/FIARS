...
def test_parse_single_row():
    ...
    assert len(rows) == 1
    row = rows[0]
    assert row["ticket_no"] == "SHGD0009000002"
    assert row["server_sn"] == "21X100001"
...
def test_parse_multi_row_same_ticket():
    ...
    assert len(rows) == 2
    assert all(r["ticket_no"] == "SHGD0009000003" for r in rows)
    assert all(r["server_sn"] == "21X100006" for r in rows)
    cats = {r["category"] for r in rows}
    assert cats == {"Board", "Memory"}


def test_parse_ignores_shift_banner_preamble():
    banner = "Taylor | Riley | Alex – BDC02 @ 9:30 am\n" + DISPATCH_ROW_HDD
...
def test_merge_overrides_ticket_number_and_fills_pn():
    ...
    assert job["ticket_number"] == "SHGD0009000002"   # dispatch table wins
...
def test_merge_disambiguates_two_parts_by_category():
    ...
    memory_job = {"server_sn": "21X100006", "category": "Memory", "part": {}}
    rows = parse_dispatch_table(DISPATCH_TABLE_TWO_PARTS)
...
def test_end_to_end_report_reflects_dispatch_merge():
    ...
    assert "Ticket Number: SHGD0009000002" in report
...
def test_draft_and_report_combine_both_parts_into_one_report():
    ...
    assert "Ticket Number: SHGD0009000004" in report
...
def test_two_tags_blocks_same_ticket_combine_into_one_report():
    ...
    assert report.count("Server SN:") == 1
    assert report.count("Location:") == 1
    assert "Ticket Number: SHGD0009000005" in report
    ...
    assert "0000:37:00.0 SN: 1000000000001" in report          # block 1, from dispatch
...
def test_combined_report_does_not_merge_different_tickets():
    ...
    board_job = {"server_sn": "21X100006", "category": "Board", "part": {},
                 "ticket_number": "SHGD0009000003", "location_full": "SOMEWHERE"}
    draft_b = default_draft(board_job)

    combined = build_combined_report([draft_a, draft_b])
    assert combined.count("Ticket Number:") == 2
    assert "SHGD0009000002" in combined
    assert "SHGD0009000003" in combined
...
def test_multi_unit_row_matches_each_same_category_job_without_being_exhausted():
    ...
    assert sns == {"802C0F0000001AC", "802C0F0000002AE"}
...
Alex Deliver onsite"""
    ...
    assert len(rows) == 2
    assert rows[0]["ticket_no"] == "SHGD0009000011"
    assert rows[0]["server_sn"] == "21X100007"
    assert rows[0]["faulty_part"] == "Memory"
    assert rows[0]["old_pn"] == "V0040C90000000ZY"
    assert rows[0]["new_pn"] == "V0040C0000000000"
    assert rows[1]["ticket_no"] == "SHGD0009000012"
    assert rows[1]["server_sn"] == "21X100008"
...
Taylor Deliver onsite"""
    ...
    assert row["engineer"] == "Taylor Deliver onsite"
...
Taylor Deliver onsite"""
    ...
    assert len(rows) == 2
    assert rows[0]["server_sn"] == "21X100001"  # first row unaffected
    second = rows[1]
    assert second["ticket_no"] == "SHGD0009000008"
