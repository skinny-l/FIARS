...
You're at **Milestone 2 (parse + store)** today. Here's the path forward. Each phase is shippable
on its own and adds visible value.

### 12.1 Phase 3 — MVP: the recommendation loop (no ML) ⭐
...
- [ ] Migrate schema to §6 (catalogs, components, notes, FTS5).
- [ ] `backfill_catalog.py`: map existing root-cause/resolution text → catalog IDs (human confirms ambiguous).
- [ ] `retrieval/lexical.py`: FTS5/BM25 over presenting fields.
- [ ] `ranking/scorer.py`: frequency + recency + success weighting, confidence + smoothing (§7.3).
- [ ] `recommend.py`: orchestrate retrieve → rank → structured result.
- [ ] Flask "**Diagnose**" panel: on parse, show ranked causes/fixes with %, supporting case IDs, note snippets.
- [ ] `generate_synthetic.py` + seed data so it demos instantly.
...
### 12.2 Phase 4 — Semantic + hybrid + evaluation

- [ ] `retrieval/semantic.py`: MiniLM embeddings (presenting fields only) → FAISS.
- [ ] `scripts/build_index.py`: build/refresh index; auto-embed on save.
- [ ] `retrieval/hybrid.py`: RRF fusion of lexical + semantic.
- [ ] `eval/leave_one_out.py`: top-1/top-3/MRR; tune `τ, ρ, α, k, K`.
- [ ] Put the eval headline number in the README.

### 12.3 Phase 5 — Analytics & dashboard

- [ ] `analytics/metrics.py`: MTTR, recurrence rates, top-N, trends.
- [ ] Streamlit dashboard (§9), all six pages.
- [ ] `start_dashboard.bat`.
