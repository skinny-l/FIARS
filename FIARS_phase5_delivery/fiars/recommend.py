...
def diagnose(path: str, query_text: str, raw_text: str = "",
             k: int = 50, max_results: int = 5,
             exclude_id: int | None = None) -> dict[str, Any]:
    """Return ranked causes + resolutions + supporting evidence + KB matches.

    exclude_id: omit a case from its own retrieval (used by the leave-one-out
    eval harness). Everyday callers (server.py) never pass this.
    """
    # Case-based retrieval: hybrid (lexical + semantic) when an embedding
    # index exists, transparently lexical-only otherwise.
    cases = hybrid.retrieve(path, query_text, k=k, exclude_id=exclude_id)
